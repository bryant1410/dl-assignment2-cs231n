...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    # the out variable.                                                         #
    #############################################################################
    out = (x - bn_param['running_mean']) * gamma / (np.sqrt(bn_param['running_var'] + eps)) + beta
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  else:
...
def batchnorm_backward(dout, cache):
  ...
  # Step 8. xg = xn * gamma
  dxn = dxg * gamma
  dgamma = np.sum(dxg * xn, axis=0, keepdims=True)

  # Step 7. xn = xc * invv
  dxc1 = dxn * invv
  dinvv = np.sum(dxn * xc, axis=0, keepdims=True)
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    ###########################################################################+
    mask = (np.random.sample(x.shape) >= p)
    out = mask * x
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for n in xrange(N):
    for f in xrange(F):
      for h_ in xrange(H_):
        for w_ in xrange(W_):
          out[n, f, h_, w_] = np.sum(w[f, :, :, :] * x_padded[n, :, (h_ * stride):(h_ * stride + HH),
                                                              (w_ * stride):(w_ * stride + WW)]) + b[f]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
...
def conv_backward_naive(dout, cache):
  ...
  for n in xrange(N):
    for f in xrange(F):
      for h_ in xrange(H_):
        for w_ in xrange(W_):
          dx[n, :, (h_ * stride):(h_ * stride + HH), (w_ * stride):(w_ * stride + WW)] += \
              dout[n, f, h_, w_] * w[f, :, :, :]
          dw[f, :, :, :] += \
              dout[n, f, h_, w_] * x_padded[n, :, (h_ * stride):(h_ * stride + HH), (w_ * stride):(w_ * stride + WW)]
...
def max_pool_forward_naive(x, pool_param):
  ...
  out = np.ndarray((N, C, H_, W_))

  for h_ in xrange(H_):
    for w_ in xrange(W_):
      out[:, :, h_, w_] = np.max(x[:, :, (h_ * stride):(h_ * stride + PH), (w_ * stride):(w_ * stride + PW)],
                                 axis=(2, 3))
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in xrange(N):
    for c in xrange(C):
      for h_ in xrange(H_):
        for w_ in xrange(W_):
          x_tile = x[n, c, (h_ * stride):(h_ * stride + PH), (w_ * stride):(w_ * stride + PW)]
          i_max, j_max = np.unravel_index(np.argmax(x_tile), x_tile.shape)
          dx[n, c, i_max + h_ * stride, j_max + w_ * stride] = dout[n, c, h_, w_]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
